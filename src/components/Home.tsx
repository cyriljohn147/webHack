            ...
            {quizzes.map((quiz, index) => {
              if (index === stage) {
                return (
                  <a
                    key={quiz.title}
                    className="flex h-16 cursor-pointer bg-green-600 items-center gap-4 rounded-xl p-3 drop-shadow-sm transition-all duration-200 ease-in-out hover:opacity-75 dark:bg-navy sm:h-20 sm:gap-8 sm:rounded-3xl xl:h-24 xl:w-[564px] xl:p-5"
                    onClick={() => {
                  ...
                  >
                    ...
                    <p className="text-lg text-white font-medium sm:text-[28px]">
                      {quiz.title}
                    </p>
                  </a>
                );
